...
def replace(i, s):
    global codestring
    codestring = codestring[:i] + s + codestring[i + len(s):]

import shiftops
#codestring = shiftops.shift_ops(codestring_works, 180, 1)

replace(3, '\x09')
replace(4, '\x09\x09')
replace(80, '\x09\x09\x09')
replace(67, '\x09'*3)
replace(62, '\x02')
replace(178, '\x09'*3)
replace(220, '\x09'*3)
replace(451, '\x09'*1)
codestringz = shiftops.shift_ops(codestring, 3, 5)
